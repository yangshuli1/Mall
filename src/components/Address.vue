    ...
    <div class="i_bg bg_color">
      ...
      <div class="m_content">
        ...
        <div class="m_right">
          ...
          <table border="0" class="add_tab" style="width:930px;"  cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td colspan="3" style="font-family:'宋体';">
                <select class="jj" name="province" v-on:change="str()" id="action">
                  <option value="0" selected="selected">请选择...</option>
                  <option value="0" selected="selected" v-for="ress in data" v-bind:value="ress.area_id" >{{ress.area_name}}</option>
                </select>
                <select class="jj" name="city" v-on:change="str1()" id="acti">
                  <option value="0" selected="selected">请选择...</option>
                  <option value="0" selected="selected" v-for="res in pare" v-bind:value="res.area_id" >{{res.area_name}}</option>

                </select >
                <select class="jj" name="area" id="act">
                  <option value="0" selected="selected">请选择...</option>
                  <option value="0" selected="selected" v-for="re in par" v-bind:value="re.area_id" >{{re.area_name}}</option>
          ...
          <p align="right">
            <a href="#">删除</a>&nbsp; &nbsp; <a href="#" class="add_b" v-on:click="str2">保存并设为默认地址</a>
          </p>
...
<<script>
  export default {
    ...
    methods: {
      str(){
        var pid= $('#action option:selected').val()
        // console.log(pid)
        axios.post(this.url+'/address',{
          pid: pid,
          token: localStorage.getItem("token")
        })
          .then(response=> {
            this.pare=response.data
            // console.log(this.pare)
          })
      },
      str1(){
        var pid= $('#acti option:selected').val()
        // console.log(pid)
        axios.post(this.url+'/address',{
          pid: pid,
          token: localStorage.getItem("token")
      ...
      str2(){
        var province= $('#action option:selected').html()
        var city= $('#acti option:selected').html()
        var county= $('#act option:selected').html()
        var address=province+"省"+city+"市"+county
        // console.log(address)
        axios.post(this.url+'/ress',{
          address: address,
          token: localStorage.getItem("token")
        })
          .then(response=> {
